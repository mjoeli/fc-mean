    ...
    def app_flow(self):
        ...
        while True:
            ...
            if state == state_local_computation:
                ...
                data_to_send = jsonpickle.encode([self.client.weighted_mean, self.client.weight])                  
            ...
            if state == state_global_aggregation:
                ...
                if len(self.data_incoming) == len(self.clients):
                    decoded_data = [jsonpickle.decode(client_data) 
                                    for client_data in self.data_incoming]
                    local_means = [local_means[0] for local_means in decoded_data]
                    weights = [weights[1] for weights in decoded_data]
                    self.data_incoming = []
                    global_mean = self.client.compute_global_mean(local_means, weights)
                    print(f'global mean: {global_mean}')
                    self.client.set_global_mean(global_mean)
